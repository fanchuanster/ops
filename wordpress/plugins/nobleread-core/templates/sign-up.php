...
<main class="nr-sign-up">
    <h1><?php esc_html_e('Create your free account', 'nobleread-core'); ?></h1>
    <p class="nr-sign-up-sub">
        <?php esc_html_e('Reading and downloading is free — an account just keeps your daily limit tied to you, not your browser.', 'nobleread-core'); ?>
    </p>

    <?php if (isset($_GET['oauth_error'])) : ?>
        <p class="nr-form-error"><?php esc_html_e('That sign-up attempt was cancelled or ran into a problem. Please try again, or use the form below.', 'nobleread-core'); ?></p>
    <?php endif; ?>
    ...
    <?php if ($google_url) : ?>
        <a class="nr-google-btn" href="<?php echo esc_url($google_url); ?>">
            <?php esc_html_e('Continue with Google', 'nobleread-core'); ?>
        </a>
        <div class="nr-sign-up-divider"><span><?php esc_html_e('or', 'nobleread-core'); ?></span></div>
    <?php endif; ?>
    ...
    <form method="post" action="<?php echo esc_url(home_url('/sign-up/')); ?>" class="nr-sign-up-form">
        ...
        <p>
            <label for="nr_name"><?php esc_html_e('Name', 'nobleread-core'); ?></label>
            <input type="text" id="nr_name" name="nr_name" value="<?php echo esc_attr($posted_name); ?>" required>
        </p>
        <p>
            <label for="nr_email"><?php esc_html_e('Email', 'nobleread-core'); ?></label>
            <input type="email" id="nr_email" name="nr_email" value="<?php echo esc_attr($posted_email); ?>" required>
        </p>
        <p>
            <label for="nr_password"><?php esc_html_e('Password', 'nobleread-core'); ?></label>
            <input type="password" id="nr_password" name="nr_password" minlength="8" required>
        </p>
        <p>
            <button type="submit" class="nr-cta"><?php esc_html_e('Create account', 'nobleread-core'); ?></button>
        </p>
    </form>
    ...
    <p class="nr-sign-up-login">
        ...
        printf(
            wp_kses(
                ...
                __('Already have an account? <a href="%s">Log in</a>.', 'nobleread-core'),
                ['a' => ['href' => []]]
            ),
