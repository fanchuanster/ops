...
def test_claims_a_cover_job():
    """A cover carries its own two fields and no phase-2 master."""
    poller = poller_answering(
        serving(
            {
                "book_id": "7",
                "kind": "cover",
                "source_key": "books/7/book/original.pdf",
                "source_format": "pdf",
                "cover_key": "books/7/cover.jpg",
                "formats": [],
                "title": "論語別裁",
            }
        )
    ...
    job = poller.claim()
    assert job is not None
    assert job.kind is JobKind.COVER
    assert job.source_format == "pdf"
    assert job.cover_key == "books/7/cover.jpg"


def test_reports_a_cover_as_a_cover_and_not_an_artifact():
    """The key it wrote, and nothing that would move the book.

    A cover arriving as an artifact would be refused — the completion
    route rejects an empty artifact list — and a cover reported as a
    phase would publish or fail a whole book over a picture.
    """
    seen: dict = {}
    ...
    poller = poller_answering(handler)
    job = Job(source_key="books/7/book/original.pdf", book_id="7", kind=JobKind.COVER)
    job.cover = "books/7/cover.jpg"
    job.advance(JobState.COMPLETED)

    poller.report(job)

    assert seen["kind"] == "cover"
    assert seen["state"] == "completed"
    assert seen["cover_key"] == "books/7/cover.jpg"
    assert "artifacts" not in seen


def test_reports_a_cover_that_could_not_be_rendered():
    seen: dict = {}
    ...
    poller = poller_answering(handler)
    job = Job(source_key="books/7/book/book.epub", book_id="7", kind=JobKind.COVER)
    job.advance(JobState.FAILED, error="this EPUB declares no cover image")

    poller.report(job)

    assert seen["kind"] == "cover"
    assert seen["state"] == "failed"
    assert seen["message"] == "this EPUB declares no cover image"
