...
Only the scanned path costs hours. The other three are near-instant,
...
def load_source(
    ...
    *,
    ...
    A scanned PDF is refused here rather than silently OCR'd: OCR takes
    hours, needs a cache directory and reports progress, so it belongs to
    the `convert` command and not to a function that otherwise returns in
    milliseconds. Callers are told which command to use instead.
    """
    kind = kind or detect(path)

    if kind is SourceKind.PDF_SCANNED:
        raise UnsupportedSource(
            f"{path.name} is a scanned PDF with no text layer, so it needs OCR. "
            "Use `converter convert` for that — it caches the read and reports "
            "progress, because OCR takes hours rather than milliseconds."
        )

    if kind is SourceKind.PDF_TEXT:
        from .pdf_text import read_pdf_text

        document, report = read_pdf_text(path, title=title, author=author)
        notes = [
